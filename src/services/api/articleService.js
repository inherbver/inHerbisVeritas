...
class ArticleService {
  ...
  async getArticles(options = {}) {
    try {
      if (this.useMockData) {
        ...
        // Pagination
        const start = options.page ? (options.page - 1) * (options.limit || 10) : 0;
      ...
      return {
        data: [],
        count: 0,
        error: handleSupabaseError(error, 'Erreur lors de la récupération des articles')
      };
    }
  }
  ...
  async getArticleById(id) {
    try {
      if (this.useMockData) {
        const article = mockArticles.find(article => article.id === Number(id));
        const enrichedArticle = article ? this.enrichArticleWithProductData(article) : null;
      ...
      return {
        data: null,
        error: handleSupabaseError(error, `Erreur lors de la récupération de l'article ${id}`)
      };
    }
  }
  ...
  async createArticle(articleData) {
    try {
      ...
      return {
        data: null,
        error: handleSupabaseError(error, "Erreur lors de la création de l'article")
      };
    }
  }
  ...
  async updateArticle(id, articleData) {
    try {
      if (this.useMockData) {
        ...
        const index = mockArticles.findIndex(article => article.id === Number(id));
        ...
        const enrichedArticle = this.enrichArticleWithProductData(updatedArticle);
        return { data: enrichedArticle, error: null };
      } else {
        const { data, error } = await supabase
          .from(TABLES.ARTICLES)
      ...
      return {
        data: null,
        error: handleSupabaseError(error, `Erreur lors de la mise à jour de l'article ${id}`)
      };
    }
  }
  ...
  async deleteArticle(id) {
    try {
      ...
      return {
        success: false,
        error: handleSupabaseError(error, `Erreur lors de la suppression de l'article ${id}`)
      };
    }
  }
  ...
  async getFeaturedArticles(limit = 3) {
    try {
      ...
      return {
        data: [],
        error: handleSupabaseError(error, 'Erreur lors de la récupération des articles mis en avant')
      };
    }
  }
  ...
  async toggleFeatured(id, featured) {
    try {
      ...
      return {
        success: false,
        error: handleSupabaseError(error, `Erreur lors de la modification du statut "mis en avant" de l'article ${id}`)
      };
    }
  }
}
